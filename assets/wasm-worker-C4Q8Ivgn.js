...
function s(n, t = globalThis, i = ["*"]) {
	t.addEventListener("message", function o(l) {
		...
		const { id: u, type: d, path: p } = Object.assign({ path: [] }, l.data), f = (l.data.argumentList || []).map(h);
		let m;
		try {
			...
			switch (d) {
				case "GET":
					...
					break;
				case "SET":
					t[p.slice(-1)[0]] = h(l.data.value), m = !0;
					break;
				case "APPLY":
					m = r.apply(t, f);
					break;
	...
	return n ? "angle_spt" === n.execution : "angle_hasib" === function(e) {
		...
		switch (n.grammarId) {
			case "count_cell": {
				...
				const i = j(t, "implementation_variant");
				return {
					family: "count_hasib",
					scorer: e,
...
function Z(e) {
	...
	if (e.algorithm && J.has(e.algorithm) && (n.classifier = e.algorithm), null != e.epochLengthSeconds && (n.epoch_length_seconds = e.epochLengthSeconds), null != e.onsetMinConsecutiveSleep && (n.onset_min_consecutive_sleep = e.onsetMinConsecutiveSleep), null != e.offsetMinConsecutiveMinutes && (n.offset_min_consecutive_minutes = e.offsetMinConsecutiveMinutes), e.scorerPostprocessing && (n.scorer_postprocessing = e.scorerPostprocessing), e.nonwearDetector && "choi_2011" !== e.nonwearDetector && (n.nonwear_detector = e.nonwearDetector), e.sleepPeriodDetection) {
		const t = U(e.sleepPeriodDetection);
...
async function ie() {
	const e = await async function(e) {
		...
		loadThreaded: () => import("./actours-Gh6dgZBm.js"),
		loadSingle: () => import("./actours-DfMu8h8Q.js"),
		warn: (e, n) => {
			console.warn(e, n);
		}
	});
...
function ce(e) {
	...
	const d = se(r.features), p = d && Array.isArray(d.feature_values) ? d : null, f = se(r.legacy_complexity_features) ?? (null === p ? d : null) ?? {}, m = n.computed_at;
	return {
...
const me = 1073741824, ye = "actours.compute.v1", _e = "actours-corrected-3.3.7-v2";
function ge(e) {
...
function be() {
	return we || (we = (async () => {
		const { mod: e, runtime: n } = await ie();
		return "configureComputeMemoryBudgetV1" in e && "function" == typeof e.configureComputeMemoryBudgetV1 && e.configureComputeMemoryBudgetV1(function() {
			...
			if ("number" != typeof e || !Number.isFinite(e) || e <= 0) return me;
			const n = Math.floor(1024 * e * 1024 * 1024 / 4);
			return Math.min(2147483648, Math.max(me, n));
		}()), e;
	})().catch((e) => {
		throw we = null, e;
	})), we;
...
function Ie(e, n, t, r) {
	...
	try {
		...
		for (const t of n) i[t] = ke(e.results.find((e) => e.algorithm === t), a);
	} catch {
...
"undefined" != typeof self && "undefined" == typeof window && (function() {
	...
	async streamParseFinishChunk() {
		...
		const n = new Float64Array(e.timestampsMs), t = new Float64Array(e.axisX), r = new Float64Array(e.axisY), a = new Float64Array(e.axisZ), i = new Float64Array(e.vectorMagnitude), o = new Float64Array(e.temperature), s = new Uint32Array(e.counts), c = new Uint32Array(e.tempCounts), l = new Float64Array(e.timestampsMs5s), u = new Float64Array(e.enmo5s), d = new Float64Array(e.anglez5s), p = new Float64Array(e.anglex5s ?? 0), f = new Float64Array(e.angley5s ?? 0), m = new Float64Array(e.mad5s ?? 0), y = new Float64Array(e.enmoa5s ?? 0), g = e, h = new Float64Array(g.zcx60s ?? []), w = new Float64Array(g.zcy60s ?? []), b = new Float64Array(g.zcz60s ?? []), S = new Uint32Array(e.counts5s), v = e, A = new Float64Array(v.mimsUnit ?? []), M = new Float64Array(v.mimsUnitX ?? []), E = new Float64Array(v.mimsUnitY ?? []), x = new Float64Array(v.mimsUnitZ ?? []), k = e;
		return _({
			timestampsMs: n,
			axisX: t,
			axisY: r,
	...
	async neishabouriCounts(e, n, t, r, a) {
		const i = (await be()).neishabouriCounts(e, n, t, r, a), o = "object" == typeof i && null !== i ? i : null, s = o ? ve(o.x) : null, c = o ? ve(o.y) : null, l = o ? ve(o.z) : null, u = o ? ve(o.vm) : null;
	...
	async zeroCrossingCounts(e, n, t, r, a) {
		const i = (await be()).zeroCrossingCounts(e, n, t, r, a), o = "object" == typeof i && null !== i ? i : null, s = o ? ve(o.zcx) : null, c = o ? ve(o.zcy) : null, l = o ? ve(o.zcz) : null;
	...
	async computeMimsUnit(e, n, t, r, a = {}) {
		...
		const o = Se(i.computeMimsUnit(e, n, t, r, a), "computeMimsUnit");
		return _(o, [
			o.mimsUnit.buffer,
			o.mimsUnitX?.buffer,
			o.mimsUnitY?.buffer,
	...
	async computeMimsUnitDataframe(e, n, t, r, a = {}) {
		...
		const o = Se(i.computeMimsUnitDataframe(e, n, t, r, a), "computeMimsUnitDataframe");
		return _(o, [
			o.mimsUnit.buffer,
			o.mimsUnitX?.buffer,
			o.mimsUnitY?.buffer,
	...
	async classifyActimetricPreschoolWristRf(e, n, t, r) {
		...
		return _(o, [o.classes.buffer]);
	},
	async lstmSpectralFeatures30s(e, n, t, r) {
		const a = await be(), i = a.lstmSpectralFeatures30s ?? a.spectralFeatures30s;
	...
	computeFileSignals: async (e) => function(e, n) {
		...
		return function(e) {
			...
			if (!Array.isArray(r)) return { byDate: n };
			for (const a of r) {
				const e = Ee(a), r = e?.analysis_date, i = e?.epoch_length_seconds;
			...
			const n = oe(e.config), t = oe(n?.signals) ?? n;
			return {
				...t ? { config: t } : {},
				days: e.days
			};
	...
	async actiwareIntervals(e, n, t) {
		const r = await be(), a = r.generateActiwareRestIntervals, i = r.actiwareSleepIntervals;
	...
	async actiwareIntervalStatistics(e) {
		const n = (await be()).actiwareIntervalStatistics;
	...
	async runScoredVariantBatch(e) {
		...
		let a = 0, i = !1;
		const o = Object.keys(e.classifierOutputs).filter((n) => e.classifierOutputs[n]?.length === e.timestamps.length).sort(), s = new Set(o), c = Object.keys(e.nonwearMasks).filter((n) => null != e.nonwearMasks[n]?.mask).sort(), l = e.variants.filter((n) => {
		...
		if (0 === u) {
		...
		} else if (l.length) if ("function" != typeof n.placeMarkersBatch) i = !0;
		else {
			const a = new Uint8Array(o.length * u);
			...
			const i = new Uint8Array(c.length * u);
			...
			const s = function(e) {
				let n;
				if ("variant" in e) {
					const { variant: a, timestamps: i, activityCounts: o, sleepScores: s, analysisDate: c, epochLengthSeconds: l, hdczaWindow: u, diary: d, onsetMinConsecutiveSleep: p, offsetMinConsecutiveMinutes: f, choiNonwear: m, eventMarkerTimestamps: y, actiwareRestWindow: _ } = e, g = a.config.rescoring === S && (r = S) === S ? { preset: r } : void 0;
					n = {
						timestamps: i,
						activityCounts: o,
						sleepScores: s,
				...
				const a = {
					...
					for (const [a, o] of n.entries()) t[a + 1] = t[a] + o.length;
					...
					if (i !== n[0]) for (const [a, o] of n.entries()) i.set(o, t[a]);
					return [i, t];
				}), s = {
					config: a.config,
					days: a.days.map((e) => Object.fromEntries(Object.entries(e).filter(([e]) => !i.includes(e))))
				};
			...
			}), d = JSON.parse(s[0]), p = d.days[0], f = n.placeMarkersBatch(JSON.stringify({
				...d.config,
				...p,
				classifier_ids: o,
				nonwear_ids: c
			}), Float64Array.from(e.timestamps), Float64Array.from(e.activityCounts), a, i, JSON.stringify(l.map((e) => ({
				id: e.id,
			...
			for (const e of f.notes) r.add(e);
			for (const [e, n] of l.entries()) t[n.id] = f.masks.slice(e * u, (e + 1) * u).buffer;
		}
		return _({
			masks: t,
			notes: [...r],
	...
	async epochWithBandpass(e, n, t) {
		const r = (await be()).epochWithBandpass(e, n, t), a = "object" == typeof r && null !== r ? r : null, i = ve(a?.timestamps), o = ve(a?.counts);
	...
	async processRawXyz(e, n, t, r, a, i) {
		const o = (await be()).processRawXyz(e, n, t, r, a, i ?? void 0), s = "object" == typeof o && null !== o ? o : null, c = s ? ve(s.enmo5s) : null, l = s ? ve(s.anglez5s) : null, u = s ? ve(s.countsX) : null, d = s ? ve(s.countsY) : null, p = s ? ve(s.countsZ) : null, f = s ? ve(s.countsVm) : null;
		if (!(c && l && u && d && p && f)) throw new Error(`processRawXyz: unexpected WASM return shape — got ${JSON.stringify(o)}`);
		const m = (e) => ve(e) ?? /* @__PURE__ */ new Float64Array(0), y = s ? m(s.anglex5s) : /* @__PURE__ */ new Float64Array(0), g = s ? m(s.angley5s) : /* @__PURE__ */ new Float64Array(0), h = s ? m(s.mad5s) : /* @__PURE__ */ new Float64Array(0), w = s ? m(s.enmoa5s) : /* @__PURE__ */ new Float64Array(0);
		return _({
			enmo5s: c,
			anglez5s: l,
			anglex5s: y,
	...
	}((await be()).getComputeCapabilitiesV1(), "74a8c9e0b681b60c6774d293093d176d7e45bdb165465bf48f44b3d56251bffc"),
	...
	async readGgirMeta(e) {
		...
		const t = n.readGgirMeta(e), r = "object" == typeof t && null !== t ? t : null, a = r ? ve(r.timestampsMs) : null, i = r ? ve(r.enmo5s) : null, o = r ? ve(r.anglez5s) : null, s = r ? ve(r.anglex5s) : null, c = r ? ve(r.angley5s) : null, l = r ? Ae(r.invalidShort) : null, u = r ? Ae(r.nonwearShort) : null, d = r ? Ae(r.part3Sib5s) : null, p = r ? ve(r.anglez5sImputed) : null, f = r ? ve(r.enmo5sImputed) : null, m = r?.part3SptNights, y = Array.isArray(m) ? m : null;
	...
	identifyGgirRData: async (e) => (await be()).identifyGgirRData(e),
	async reviewGgirResults(e) {
		const n = (await be()).reviewGgirResults(e.part1, e.configCsv, e.ms2, e.ms3, e.ms4, e.ms5, e.sleeplogRData, e.sleeplogCsv, e.recordingId, e.edits, e.crossCheck);
		return _(n, [n.sib.buffer, n.invalid.buffer]);
	},
