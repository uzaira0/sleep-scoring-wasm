...
async function bt(e) {
	if (void 0 !== gt) return gt;
	void 0 !== e && (Object.getPrototypeOf(e) === Object.prototype ? {module_or_path: e} = e : console.warn("using deprecated parameters for the initialization function; pass a single object instead")), void 0 === e && (e = new URL("/sleep-scoring-wasm/assets/actours_bg-DPhpOfU9.wasm", "" + import.meta.url));
	const n = Bn();
...
"undefined" != typeof self && "undefined" == typeof window && (function() {
	...
	async runScoredVariantBatch(e) {
		...
		let o = 0, i = !1;
		const a = Object.keys(e.classifierOutputs).filter((n) => e.classifierOutputs[n]?.length === e.timestamps.length).sort(), s = new Set(a), c = Object.keys(e.nonwearMasks).filter((n) => null != e.nonwearMasks[n]?.mask).sort(), l = e.variants.filter((n) => {
			if (!s.has(n.classifierConfigId)) return o += 1, !1;
		...
		if (0 === u) {
			...
			}), _ = JSON.parse(s[0]), f = _.days[0], d = n.placeMarkersBatch(JSON.stringify({
				..._.config,
				...f,
				classifier_ids: a,
				nonwear_ids: c
			}), Float64Array.from(e.timestamps), Float64Array.from(e.activityCounts), o, i, JSON.stringify(l.map((e) => ({
				id: e.id,
			...
			for (const [e, n] of l.entries()) t[n.id] = d.masks.slice(e * u, (e + 1) * u).buffer;
		}
		return b({
			masks: t,
			notes: [...r],
	...
	}((await Wt()).getComputeCapabilitiesV1(), "cb9eedb17cfcf01e0b0dc2646ad5eabc049c829b0c183f56ca3c56d19b04a733"),
