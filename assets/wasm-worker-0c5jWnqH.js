...
let $ = null;
const L = /unreachable|RuntimeError|out of bounds|wasm/i, H = /* @__PURE__ */ new WeakSet();
async function V() {
	const e = await async function(e) {
		if (!0 !== (t = e.scope).pinnedSingleThread && t.sharedArrayBuffer && t.crossOriginIsolated && t.cores > 1) {
			let t;
			try {
				const n = await e.loadThreaded();
				if (t = n, H.has(n)) throw new Error("thread pool start was abandoned earlier in this worker");
				await n.default();
				const r = Math.min(4, e.scope.cores);
				return n.threadPoolReady() || await function(e, t) {
					let n;
				...
				}(n.startThreadPool(r), e.poolStartTimeoutMs ?? 15e3), {
					mod: n,
					runtime: {
						threaded: !0,
						threads: r
				...
				void 0 !== t && H.add(t), e.warn("[wasm] threaded runtime unavailable, using the single-thread package", r);
			}
		}
		var t;
	...
	return function(e) {
		...
		if (16 !== n.length) throw new Error(`WASM integrity check failed: scoreSadeh length ${String(n.length)}, expected ${String(16)}`);
		for (let r = 0; r < 16; r++) if (1 !== n[r]) throw new Error(`WASM integrity check failed: scoreSadeh(zeros)[${String(r)}] = ${String(n[r])}, expected 1. Bundle may be corrupt — clear the site cache and reload.`);
	}(e.mod), e;
}
function J(e) {
...
function Y(e) {
	const t = X(e), n = t?.analysis_date, r = X(t?.intrinsic), a = r?.state;
	...
	const f = X(r.features), p = f && Array.isArray(f.feature_values) ? f : null, d = X(r.legacy_complexity_features) ?? (null === p ? f : null) ?? {}, m = t.computed_at;
	return {
...
function K(e, t) {
	const n = Array.isArray(e) ? e : [], r = (e) => q(n[e]) ?? t;
	return [
		r(0),
		r(1),
		r(2)
...
function Q(e, t) {
	...
	return {
		scale: K(n.scale, t ? 1 : 0),
		offset: K(n.offset, 0),
		temperatureOffset: K(n.temperatureOffset, 0),
		errorStart: q(n.errorStart),
		errorEnd: q(n.errorEnd),
		fitAttempted: !0 === n.fitAttempted,
		numPoints: Math.max(0, Math.round(q(n.numPoints) ?? 0)),
		hoursUsed: q(n.hoursUsed) ?? 0,
		success: !0 === n.success,
...
function ee(e) {
	...
	if ("string" != typeof n || !Z.includes(n)) return null;
	const r = Q(t, !0);
	return {
		...r,
		disposition: n,
...
const te = 1073741824, ne = "actours.compute.v1", re = "actours-corrected-3.3.7-v2";
function ae(e) {
	return "number" == typeof e ? e : "nan" === e ? NaN : "positive_infinity" === e ? Number.POSITIVE_INFINITY : Number.NEGATIVE_INFINITY;
}
function oe(e) {
	return null === e ? null : ae(e);
}
let se = null;
function ie() {
	return se || (se = (async () => {
		const { mod: e, runtime: t } = await V();
		return "configureComputeMemoryBudgetV1" in e && "function" == typeof e.configureComputeMemoryBudgetV1 && e.configureComputeMemoryBudgetV1(function() {
			...
			if ("number" != typeof e || !Number.isFinite(e) || e <= 0) return te;
			const t = Math.floor(1024 * e * 1024 * 1024 / 4);
			return Math.min(2147483648, Math.max(te, t));
		}()), e;
	})().catch((e) => {
		throw se = null, e;
	})), se;
}
function ce(e, t = "computeMimsUnit") {
	if ("object" != typeof e || null === e) throw new Error(`${t}: unexpected WASM return shape — got ${typeof e}`);
	const n = e, r = ue(n.mimsUnit);
...
function ye(e, t, n, r) {
	...
	try {
		...
		for (const n of t) o[n] = me(e.results.find((e) => e.algorithm === n), a);
	} catch {
...
"undefined" != typeof self && "undefined" == typeof window && (function() {
	...
	async detectNonwearUnified(e) {
		const t = function(e, t) {
			return ye(e, [t.algorithm], t.signals, t.config)[t.algorithm];
		}(await ie(), e);
		return g(t, t.nonwear ? [t.nonwear.buffer] : []);
	},
	async detectNonwearUnifiedBatch(e, t) {
		const n = ye(await ie(), e, t);
	...
	async scoreSadeh(e, t) {
		const n = await ie(), r = new Uint8Array(n.scoreSadeh(e, t));
		return g(r, [r.buffer]);
	},
	async scoreColeKripke(e, t) {
		const n = await ie(), r = new Uint8Array(n.scoreColeKripke(e, t));
		return g(r, [r.buffer]);
	},
	async detectNonwear(e) {
		const t = await ie(), n = new Uint8Array(t.detectNonwear(e));
		return g(n, [n.buffer]);
	},
	async detectNonwearChoi2011(e, t = 60) {
		const n = await ie(), r = new Uint8Array("function" == typeof n.detectNonwearChoi2011Epoch ? n.detectNonwearChoi2011Epoch(e, t) : n.detectNonwearChoi2011(e));
		return g(r, [r.buffer]);
	},
	parseActigraphCsv: async (e, t) => fe((await ie()).parseActigraphCsv(e, t), "parseActigraphCsv"),
	parseGeneactivCsv: async (e) => fe((await ie()).parseGeneactivCsv(e), "parseGeneactivCsv"),
	isGeneactivFormat: async (e) => (await ie()).isGeneactivFormat(e),
	async csvBufferClear(e) {
		(await ie()).csvBufferClear(e);
	},
	async csvBufferAppend(e) {
		(await ie()).csvBufferAppend(e);
	},
	parseGeneactivCsvBuffered: async () => fe((await ie()).parseGeneactivCsvBuffered(), "parseGeneactivCsvBuffered"),
	parseActigraphCsvBuffered: async (e) => fe((await ie()).parseActigraphCsvBuffered(e), "parseActigraphCsvBuffered"),
	async sha256Start() {
		(await ie()).sha256StreamStart();
	},
	async sha256Feed(e) {
		(await ie()).sha256StreamFeed(e);
	},
	sha256Finish: async () => (await ie()).sha256StreamFinish(),
	...
	async streamParseStartData(e, t) {
		(await ie()).streamParseStartData(e, t);
	},
	streamParseFeed: async (e) => (await ie()).streamParseFeed(e),
	async streamParseFinish() {
		const e = (await ie()).streamParseFinish();
	...
	async streamParseFinishChunk() {
		const e = (await ie()).streamParseFinishChunk();
	...
	async neishabouriCounts(e, t, n, r, a) {
		const o = (await ie()).neishabouriCounts(e, t, n, r, a), s = "object" == typeof o && null !== o ? o : null, i = s ? ue(s.x) : null, c = s ? ue(s.y) : null, u = s ? ue(s.z) : null, l = s ? ue(s.vm) : null;
	...
	async zeroCrossingCounts(e, t, n, r, a) {
		const o = (await ie()).zeroCrossingCounts(e, t, n, r, a), s = "object" == typeof o && null !== o ? o : null, i = s ? ue(s.zcx) : null, c = s ? ue(s.zcy) : null, u = s ? ue(s.zcz) : null;
	...
	async computeMimsUnit(e, t, n, r, a = {}) {
		...
		const s = ce(o.computeMimsUnit(e, t, n, r, a), "computeMimsUnit");
		return g(s, [
			s.mimsUnit.buffer,
			s.mimsUnitX?.buffer,
			s.mimsUnitY?.buffer,
	...
	async computeMimsUnitDataframe(e, t, n, r, a = {}) {
		...
		const s = ce(o.computeMimsUnitDataframe(e, t, n, r, a), "computeMimsUnitDataframe");
		return g(s, [
			s.mimsUnit.buffer,
			s.mimsUnitX?.buffer,
			s.mimsUnitY?.buffer,
	...
	async classifyActimetricPreschoolWristRf(e, t, n, r) {
		...
		const s = function(e, t = "classifyActimetricPreschoolWristRf") {
			...
			const r = "object" == typeof e && null !== e ? e : null, a = r ? le(r.classes) ?? le(r.activityClasses) ?? le(r.predictions) ?? le(r.activity) : null;
	...
	async lstmSpectralFeatures30s(e, t, n, r) {
		const a = await ie(), o = a.lstmSpectralFeatures30s ?? a.spectralFeatures30s;
	...
	async computeSleepMetrics(e, t, n) {
		const r = (await ie()).computeSleepMetrics(e, t, n);
	...
	computeFileDifficulty: async (e) => function(e, t) {
		...
		return "function" != typeof n ? (console.warn("[wasm-worker] computeNightDifficulty is not present in this WASM bundle — night difficulty degrades to empty. Rebuild the actours WASM crate to enable it."), { byDate: {} }) : function(e) {
			const t = {}, n = /* @__PURE__ */ new Set(), r = pe(e)?.results;
	...
	computeFileSignals: async (e) => function(e, t) {
		...
		const [r, ...a] = t;
		return function(e) {
			const t = {}, n = /* @__PURE__ */ new Set(), r = pe(e)?.signals;
			if (!Array.isArray(r)) return { byDate: t };
			for (const a of r) {
				const e = pe(a), r = e?.analysis_date, o = e?.epoch_length_seconds;
			...
			const t = J(e.config), n = J(t?.signals) ?? t;
			return {
				...n ? { config: n } : {},
				days: e.days
			};
	...
	computeCircadian: async (e) => function(e, t) {
		...
		const r = n(...t);
		return r && "object" == typeof r && Array.isArray(r.day_summaries) ? r : de;
	}(await ie(), e),
	...
	async runScoredVariantBatch(e) {
		const t = await ie(), n = {}, r = /* @__PURE__ */ new Set();
		let a = 0, o = !1;
	...
	epochRawData: async (e, t, n, r, a) => fe((await ie()).epochRawData(e, t, n, r, a), "epochRawData"),
	async epochWithBandpass(e, t, n) {
		const r = (await ie()).epochWithBandpass(e, t, n), a = "object" == typeof r && null !== r ? r : null, o = ue(a?.timestamps), s = ue(a?.counts);
	...
	async processRawXyz(e, t, n, r, a, o) {
		const s = (await ie()).processRawXyz(e, t, n, r, a, o ?? void 0), i = "object" == typeof s && null !== s ? s : null, c = i ? ue(i.enmo5s) : null, u = i ? ue(i.anglez5s) : null, l = i ? ue(i.countsX) : null, f = i ? ue(i.countsY) : null, p = i ? ue(i.countsZ) : null, d = i ? ue(i.countsVm) : null;
		if (!(c && u && l && f && p && d)) throw new Error(`processRawXyz: unexpected WASM return shape — got ${JSON.stringify(s)}`);
		const m = (e) => ue(e) ?? /* @__PURE__ */ new Float64Array(0), y = i ? m(i.anglex5s) : /* @__PURE__ */ new Float64Array(0), h = i ? m(i.angley5s) : /* @__PURE__ */ new Float64Array(0), w = i ? m(i.mad5s) : /* @__PURE__ */ new Float64Array(0), b = i ? m(i.enmoa5s) : /* @__PURE__ */ new Float64Array(0);
		return g({
			enmo5s: c,
			anglez5s: u,
			anglex5s: y,
	...
	async processRawXyzImputed(e, t, n, r, a, o, s = 60) {
		const i = await ie(), c = "function" == typeof i.processRawXyzImputedWithEpoch ? i.processRawXyzImputedWithEpoch(e, t, n, r, a, o ?? void 0, s) : i.processRawXyzImputed(e, t, n, r, a, o ?? void 0), u = "object" == typeof c && null !== c ? c : null, l = u ? ue(u.enmo5s) : null, f = u ? ue(u.anglez5s) : null, p = u ? ue(u.countsX) : null, d = u ? ue(u.countsY) : null, m = u ? ue(u.countsZ) : null, y = u ? ue(u.countsVm) : null;
		if (!(l && f && p && d && m && y)) throw new Error(`processRawXyzImputed: unexpected WASM return shape — got ${JSON.stringify(c)}`);
		const h = (e) => ue(e) ?? /* @__PURE__ */ new Float64Array(0), w = u ? h(u.anglex5s) : /* @__PURE__ */ new Float64Array(0), b = u ? h(u.angley5s) : /* @__PURE__ */ new Float64Array(0), _ = u ? h(u.mad5s) : /* @__PURE__ */ new Float64Array(0), A = u ? h(u.enmoa5s) : /* @__PURE__ */ new Float64Array(0);
		return g({
			enmo5s: l,
			anglez5s: f,
			anglex5s: w,
	...
	async detectDetachFromAccelerationG(e, t) {
		const n = await ie(), r = new Uint8Array(n.detectDetachFromAccelerationG(e, t));
		return g(r, [r.buffer]);
	},
	...
	}((await ie()).detectHdcza(e, t, n ?? null, r ?? null)),
		...
		return s ? {
			result: (i = s.result, {
				metadata: {
					...
					nonwearFraction: ae(i.metadata.nonwearFraction)
				},
				days: i.days.map((e) => ({
					calendarDate: e.calendarDate,
					dayNumber: e.dayNumber,
					validHours: ae(e.validHours),
					nonwearHours: ae(e.nonwearHours),
					totalEpochs: e.totalEpochs,
					sedentaryMinutes: ae(e.sedentaryMinutes),
					lightMinutes: ae(e.lightMinutes),
					moderateMinutes: ae(e.moderateMinutes),
					vigorousMinutes: ae(e.vigorousMinutes),
					mvpaMinutes: ae(e.mvpaMinutes),
					l5ValueMg: ae(e.l5ValueMg),
					l5OnsetEpoch: e.l5OnsetEpoch,
					m5ValueMg: ae(e.m5ValueMg),
					m5OnsetEpoch: e.m5OnsetEpoch,
					igGradient: ae(e.igGradient),
					igIntercept: ae(e.igIntercept),
					igRsquared: ae(e.igRsquared),
					fragTpIn2ac: ae(e.fragTpIn2ac),
					fragTpAc2in: ae(e.fragTpAc2in),
					nFragments: e.nFragments,
					nightNumber: e.nightNumber,
					tstMinutes: oe(e.tstMinutes),
					wasoMinutes: oe(e.wasoMinutes),
					sleepEfficiency: oe(e.sleepEfficiency),
					numberOfAwakenings: e.numberOfAwakenings,
					sptDurationHours: oe(e.sptDurationHours),
	...
	getComputeIdentity: async () => function(e, t) {
		...
		if (a !== ne) throw new Error(`Unsupported Actours compute contract ${a}; expected ${ne}`);
		...
		if (!(Array.isArray(n.semanticProfiles) ? n.semanticProfiles : []).includes(re)) throw new Error(`Actours capability does not provide required semantic profile ${re}`);
	...
	}((await ie()).getComputeCapabilitiesV1(), "9ee486e333eff78a7367fa1372a9118c8405ed0a905b59b2eeb011bb06722ce8"),
	runGgirFromEpoch: async (e, t, n, r, a) => (await ie()).runGgirFromEpoch({
		anglez: e,
		enmo: t,
		sampleRateHz: n,
		startTsEpochSec: r,
	...
	async readGgirMeta(e) {
		...
		const n = t.readGgirMeta(e), r = "object" == typeof n && null !== n ? n : null, a = r ? ue(r.timestampsMs) : null, o = r ? ue(r.enmo5s) : null, s = r ? ue(r.anglez5s) : null, i = r ? ue(r.anglex5s) : null, c = r ? ue(r.angley5s) : null, u = r ? le(r.invalidShort) : null, l = r ? le(r.nonwearShort) : null;
	...
	async scoreGgirSib(e, t, n) {
		const r = (await ie()).scoreGgirSib(e, t, n);
	...
	async scoreGgirHasib(e) {
		const t = (await ie()).scoreGgirHasib(e);
	...
	async scoreGgirHasibVariant(e, t, n) {
		...
		}((await ie()).scoreGgirHasibVariant({
			data: e,
			algo: t,
			...n ? { config: n } : {}
		}), t);
	...
	async detectGgirHasptVariant(e) {
		const t = function(e, t) {
			...
			const n = e, r = le(n.nomov), a = ue(n.rollingMedian);
		...
		}((await ie()).detectGgirHasptVariant(e), e.algo);
		return g(t, [t.nomov.buffer, t.rollingMedian.buffer]);
	},
	async scoreAllDays(e) {
		const t = (await ie()).scoreAllDays(e);
