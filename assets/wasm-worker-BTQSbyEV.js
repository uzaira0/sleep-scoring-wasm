...
W.filter((e) => "planner_output_cell" === e.cellRole).map((e) => `${e.segments[0]?.literalValue ?? ""}${e.segmentSeparator}`).filter((e, n, t) => t.indexOf(e) === n).sort();
...
function Q(e) {
	...
	e.choiNonwear && (n.choi_nonwear = e.choiNonwear), e.ggirInvalid && (n.ggir_invalid = Array.from(e.ggirInvalid)), e.ggirHasptAlgo && (n.ggir_haspt_algo = e.ggirHasptAlgo), e.ggirDiaryHasBedlog && (n.ggir_diary_has_bedlog = !0), e.sensorNonwear && e.sensorNonwear.length > 0 && (n.sensor_nonwear = e.sensorNonwear.map((e) => ({
...
async function oe() {
	const e = await async function(e) {
		...
		loadThreaded: () => import("./actours-BXeRjaRQ.js"),
		loadSingle: () => import("./actours-BZXLi34a.js"),
		warn: (e, n) => {
			console.warn(e, n);
		}
	});
...
function Se(e, n = "computeMimsUnit") {
	...
	const f = o("mimsOrientationYAngle");
	f && (i.mimsOrientationYAngle = f);
	const m = o("mimsOrientationZAngle");
	return m && (i.mimsOrientationZAngle = m), i;
}
function ve(e) {
...
"undefined" != typeof self && "undefined" == typeof window && (function() {
	...
	async actiwareIntervals(e, n) {
		const t = await be(), r = t.generateActiwareRestIntervals, a = t.actiwareSleepIntervals;
		if ("function" != typeof r || "function" != typeof a) return null;
		if (n.length > 0) {
			const t = a(e, n, null);
			return {
				intervals: [...n, ...t],
	...
	async runScoredVariantBatch(e) {
		...
		if (0 === u) {
			...
			const s = function(e) {
				let n;
				if ("variant" in e) {
					const { variant: a, timestamps: o, activityCounts: i, sleepScores: s, analysisDate: c, epochLengthSeconds: l, hdczaWindow: u, diary: d, onsetMinConsecutiveSleep: p, offsetMinConsecutiveMinutes: f, choiNonwear: m, eventMarkerTimestamps: y, actiwareRestWindow: _ } = e, g = a.config.rescoring === S && (r = S) === S ? { preset: r } : void 0;
					n = {
						timestamps: o,
						activityCounts: i,
						sleepScores: s,
	...
	async processRawXyz(e, n, t, r, a, o) {
		const i = (await be()).processRawXyz(e, n, t, r, a, o ?? void 0), s = "object" == typeof i && null !== i ? i : null, c = s ? ve(s.enmo5s) : null, l = s ? ve(s.anglez5s) : null, u = s ? ve(s.countsX) : null, d = s ? ve(s.countsY) : null, p = s ? ve(s.countsZ) : null, f = s ? ve(s.countsVm) : null;
		if (!(c && l && u && d && p && f)) throw new Error(`processRawXyz: unexpected WASM return shape — got ${JSON.stringify(i)}`);
	...
	}((await be()).getComputeCapabilitiesV1(), "aff6dca57b24ac65ba9f8e29910bdbde8cb0b17c71e0a55c3e9a7f7753af1165"),
